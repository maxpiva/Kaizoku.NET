...
public class View implements Drawable.Callback, KeyEvent.Callback,
    ...
    public final int getWidth() {
        return Math.max(mRight - mLeft, 0);
    }

    @ViewDebug.ExportedProperty(category = "layout")
    public final int getHeight() {
        return Math.max(mBottom - mTop, 0);
    }
    ...
    public void postInvalidate() {
        // No-op in compatibility build. Rendering happens through host platform facilities.
    }
    ...
    public void layout(int l, int t, int r, int b) {
        final boolean changed = setFrame(l, t, r, b);
    ...
    protected boolean setFrame(int left, int top, int right, int bottom) {
        final boolean changed = left != mLeft || top != mTop || right != mRight || bottom != mBottom;
        final int oldWidth = mRight - mLeft;
        final int oldHeight = mBottom - mTop;
        final int newWidth = right - left;
        final int newHeight = bottom - top;
        if (!changed && newWidth == oldWidth && newHeight == oldHeight) {
            return false;
        }
        mLeft = left;
        mTop = top;
    ...
    public View getRootView() {
        View current = this;
        // Walk up the parent chain until we reach the top-most View.
        while (current.mParent instanceof View) {
            current = (View) current.mParent;
        }
        return current;
    }
    ...
    public void setMinimumHeight(int minHeight) {
        final int clamped = Math.max(0, minHeight);
        if (clamped != mMinHeight) {
            mMinHeight = clamped;
            requestLayout();
        }
    }
    ...
    public void setMinimumWidth(int minWidth) {
        final int clamped = Math.max(0, minWidth);
        if (clamped != mMinWidth) {
            mMinWidth = clamped;
            requestLayout();
        }
    }
    ...
    public static class MeasureSpec {
        ...
        public static int makeMeasureSpec(@IntRange(from = 0, to = (1 << MeasureSpec.MODE_SHIFT) - 1) int size,
            ...
            final int clampedSize = Math.max(0, size);
            return (clampedSize & ~MODE_MASK) | (mode & MODE_MASK);
        }

        public static int makeSafeMeasureSpec(int size, int mode) {
            return makeMeasureSpec(Math.max(0, size), mode);
        }
        ...
        static int adjust(int measureSpec, int delta) {
            ...
            final int size = Math.max(0, getSize(measureSpec) + delta);
            return makeMeasureSpec(size, mode);
        }

        public static String toString(int measureSpec) {
            final int mode = getMode(measureSpec);
            final int size = getSize(measureSpec);
            ...
            return "MeasureSpec{" + modeString + " " + size + "}";
        }
    }
